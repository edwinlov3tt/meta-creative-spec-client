  ...
  return queryOne<ElementRevision>(
    `INSERT INTO element_revisions
     (approval_request_id, participant_id, element_path, original_value,
      revised_value, comment, status, parent_revision_id, ip_address, user_agent)
...
/**
 * Get threaded revisions (comments with nested replies)
 * Returns top-level comments with their replies nested in a tree structure
 */
export async function getThreadedRevisions(
  approvalRequestId: number
): Promise<any[]> {
  // Get all revisions with participant info
  const allRevisions = await query<any>(
    `SELECT
       er.*,
       ap.email as participant_email,
       ap.name as participant_name,
       ap.role as participant_role
     FROM element_revisions er
     LEFT JOIN approval_participants ap ON er.participant_id = ap.id
     WHERE er.approval_request_id = $1
  ...
  // First pass: create map of all revisions with participant data
  allRevisions.forEach(rev => {
    revisionMap.set(rev.id, {
      ...rev,
      participant: {
        id: rev.participant_id,
        email: rev.participant_email,
        name: rev.participant_name,
        role: rev.participant_role,
      },
      replies: [],
    });
  });
  ...
  allRevisions.forEach(rev => {
    const revision = revisionMap.get(rev.id);
    if (rev.parent_revision_id) {
      // This is a reply - add to parent's replies array
      const parent = revisionMap.get(rev.parent_revision_id);
      if (parent) {
        parent.replies.push(revision);
      }
    } else {
      // This is a top-level comment
      topLevel.push(revision);
    }
  });
...
/**
 * Create a reply to an existing revision (comment)
 */
export async function createRevisionReply(
  parentId: number,
  comment: string,
  participantId: number,
  approvalRequestId: number,
  ...
  // Get parent revision to inherit element_path
  const parent = await queryOne<ElementRevision>(
    'SELECT * FROM element_revisions WHERE id = $1',
    [parentId]
  );
  ...
  // Replies inherit the element_path from parent and are always approved (they're just comments)
  return createElementRevision({
    approval_request_id: approvalRequestId,
    participant_id: participantId,
    element_path: parent.element_path,
    comment,
    status: 'approved', // Replies don't have approval status
...
export async function resolveRevision(
  ...
  return queryOne<ElementRevision>(
    `UPDATE element_revisions
     SET is_resolved = TRUE,
         resolved_at = NOW(),
         resolved_by_email = $2,
         updated_at = NOW()
     WHERE id = $1 AND parent_revision_id IS NULL
     RETURNING *`,
    [revisionId, userEmail]
  );
}
 ...
 */
export async function unresolveRevision(
  revisionId: number
): Promise<ElementRevision | null> {
  return queryOne<ElementRevision>(
    `UPDATE element_revisions
...
/**
 * Update email tracking based on webhook events from Resend
 * Handles: delivered, opened, clicked, bounced, complained
 * This function is idempotent - safe to call multiple times for same event
 */
export async function updateEmailTracking(
  ...
  try {
    ...
    const tracking = await getEmailTrackingByProviderID(emailProviderId);
    if (!tracking) {
      console.warn(`Email tracking not found for provider ID: ${emailProviderId}`);
      return null;
    }

    const requestId = approvalRequestId || tracking.approval_request_id;
    const partId = participantId || tracking.participant_id;

    // Update email_tracking table based on event type
    let updated: EmailTracking | null = null;

    switch (eventType) {
      case 'email.delivered':
        // Update metadata with delivery info
        updated = await queryOne<EmailTracking>(
          `UPDATE email_tracking
           ...
           RETURNING *`,
          [emailProviderId, JSON.stringify({ delivered_at: eventData.delivered_at || eventData.timestamp })]
        );
        break;

      case 'email.opened':
        // Update opened_at (only first time) and increment open_count
        updated = await queryOne<EmailTracking>(
          `UPDATE email_tracking
           ...
           WHERE email_provider_id = $1
           RETURNING *`,
          [emailProviderId, eventData.opened_at || eventData.timestamp]
        );

        // Log activity only on first open
        if (updated && updated.open_count === 1) {
          await createActivity({
            approval_request_id: requestId,
            participant_id: partId || undefined,
            event_type: 'email_opened',
            user_email: tracking.recipient_email,
            metadata: { source: 'webhook', ...eventData },
          });
        }
        break;

      case 'email.clicked':
        // Update clicked_at (only first time) and increment click_count
        updated = await queryOne<EmailTracking>(
          `UPDATE email_tracking
           SET clicked_at = COALESCE(clicked_at, $2),
               ...
               metadata = jsonb_set(
                 COALESCE(metadata, '{}'::jsonb),
                 '{clicks}',
                 COALESCE(metadata->'clicks', '[]'::jsonb) || $3::jsonb
               ),
               updated_at = NOW()
           WHERE email_provider_id = $1
           RETURNING *`,
          [
            emailProviderId,
            eventData.clicked_at || eventData.timestamp,
            JSON.stringify([{
              clicked_at: eventData.clicked_at || eventData.timestamp,
              url: eventData.click_url,
            }])
          ]
        );

        // Log activity only on first click
        if (updated && updated.click_count === 1) {
          await createActivity({
            approval_request_id: requestId,
            participant_id: partId || undefined,
            event_type: 'email_clicked',
            user_email: tracking.recipient_email,
            metadata: { source: 'webhook', click_url: eventData.click_url, ...eventData },
          });
        }
        break;

      case 'email.bounced':
        // Update metadata with bounce info
        updated = await queryOne<EmailTracking>(
          `UPDATE email_tracking
           SET metadata = jsonb_set(
             COALESCE(metadata, '{}'::jsonb),
           ...
           WHERE email_provider_id = $1
           RETURNING *`,
          [emailProviderId, JSON.stringify({
            bounced_at: eventData.bounced_at || eventData.timestamp,
            ...eventData
          })]
        );
      ...
      case 'email.complained':
        // Update metadata with spam complaint info
        updated = await queryOne<EmailTracking>(
          `UPDATE email_tracking
           SET metadata = jsonb_set(
             COALESCE(metadata, '{}'::jsonb),
           ...
           RETURNING *`,
          [emailProviderId, JSON.stringify({
            complained_at: eventData.complained_at || eventData.timestamp,
            ...eventData
          })]
        );
    ...
    console.error(`Error updating email tracking for ${eventType}:`, error);
    throw error;
  }
}
